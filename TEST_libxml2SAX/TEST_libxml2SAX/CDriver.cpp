...
void CDriver::OnStartElememnt( void* ctx,
...
bool CDriver::OnEndElement(void* ctx,
    const xmlChar* localname,
    const xmlChar* prefix,
    const xmlChar* URI)
{

    if (!strcmp("driver", (const char*)localname))
        return true;

    return false;
}

void CDriver::OnCharacter(void* ctx, const xmlChar* ch, int len)
{
    char str[MY_MAX] = { 0 };
